...
const App: React.FC = () => {
  ...
  const getSelectedCardData = () => {
    return currentCards.find(c => c.id === selectedCardId);
  };

  const selectedCard = getSelectedCardData();
      ...
      {state === AppState.HOME ? (
        ...
        <div className="w-full max-w-6xl flex flex-col items-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full items-stretch">
            ...
            <div
              ...
              className={`
                ...
                ${selectedCardId === -1 ? 'border-indigo-400 ring-4 ring-indigo-50 shadow-indigo-100' : ''}
              `}
            >
          ...
          <div className={`mt-10 w-full max-w-2xl transition-all duration-700 transform ${selectedCard ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
            <div className="bg-white/80 backdrop-blur-md border border-indigo-100 p-8 rounded-[2rem] shadow-lg text-center">
              ...
              <p className="text-slate-600 text-lg leading-relaxed break-keep">
                ...
                가끔은 이 막막함과 피로함이 당연한 것일지도 모릅니다. <br/>
                어떤 사연인지 리더분들께 자세하게 공유해주실 수 있을까요?
              </p>
            </div>
          </div>
          ...
          <div className="mt-12 flex flex-col items-center gap-6">
            ...
            <div className="text-slate-400 text-sm flex items-center gap-2 bg-white/50 px-5 py-2.5 rounded-full border border-slate-100 backdrop-blur-sm shadow-sm">
